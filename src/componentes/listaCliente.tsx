...
const ListaCliente: React.FC<props> = ({ tema, clientes: clientesProps, setClientes: setClientesProps, onRecarregarClientes }) => {
    ...
    const salvarEdicao = async (clienteEditado: Cliente) => {
        try {
            const clienteAPI = ClienteService.converterParaClienteAPI(clienteEditado);
            const clienteAtualizado = await ClienteService.atualizarCliente(clienteEditado.id, clienteAPI);
            const clienteConvertido = ClienteService.converterParaClienteFront(clienteAtualizado);
            
            const filtradosAtualizados = clientesFiltrados.map(cliente => 
                cliente.id === clienteEditado.id ? clienteConvertido : cliente
            );
            ...
            setClienteSelecionado(clienteConvertido);
        } catch (error) {
    ...
    const recarregarClientesDaAPI = async () => {
        if (onRecarregarClientes) {
            onRecarregarClientes();
        }
    };
